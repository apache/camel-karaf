    ...
    public String getDocLink(Object model) {
        if (localDocExists(model)) {
            ...
            if ("org.apache.camel.karaf".equals(component.getGroupId())) {
                return String.format("xref:3.9.x@ccamel-karaf::%s-component.adoc", scheme);
            } else {
                return String.format("xref:3.9.xcomponents::%s-component.adoc", scheme);
            }
        } else if (model instanceof DataFormatModel) {
            return String.format("xref:3.9.xcomponents:dataformats:%s-dataformat.adoc",
                    invokeGetter(model, "getName"));
        } else if (model instanceof LanguageModel) {
            return String.format("xref:3.9.xcomponents:languages:%s-language.adoc",
                    invokeGetter(model, "getName"));
            ...
            if ("org.apache.camel.karaf".equals(other.getGroupId())) {
                return String.format("xref:3.9.xcamel-karaf::%s.adoc", name);
            } else {
                return String.format("xref:3.9.xcomponents:others:%s.adoc", name);
            }
        } else {
            return null;
        }
