...
public class OsgiPackageScanClassResolver extends DefaultPackageScanClassResolver {
    ...
    private final static Logger LOGGER = LoggerFactory.getLogger(OsgiPackageScanClassResolver.class);
    ...
    public void find(PackageScanFilter test, String packageName, Set<Class<?>> classes) {
        ...
        if (classes.size() == classesSize) {
            ...
            LOGGER.trace("Cannot find any classes in bundles, not trying regular classloaders scanning: {}", packageName);
    ...
    private Set<String> getImplementationsInBundle(PackageScanFilter test, String packageName) {
        ...
        for (Bundle bd : bundles) {
            LOGGER.trace("Searching in bundle: {}", bd);
            try {
                ...
                while (paths != null && paths.hasMoreElements()) {
                    ...
                    urls.add(urlString);
                    LOGGER.trace("Added url: {}", urlString);
                }
            } catch (Throwable t) {
                LOGGER.warn("Cannot search in bundle: " + bundle + " for classes matching criteria: " + test + " due: "
