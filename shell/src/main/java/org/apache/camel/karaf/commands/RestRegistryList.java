...
import org.apache.camel.karaf.commands.internal.CamelController;
...
@Service
public class RestRegistryList extends CamelController implements Action {
    ...
    @Option(name = "--decode", aliases = "-d", description = "Whether to decode the endpoint uri to its human readable", required = false, multiValued = false, valueToShowInHelp = "true")
    boolean decode = true;
    ...
    public Object execute() throws Exception {
        List<Map<String, String>> services = getRestServices(name);
        if (services.isEmpty()) {
        ...
        if (services.size() > 0) {
            if (verbose) {
                System.out.println(String.format(headerFormat, URL_COLUMN_NAME, BASE_PATH_LABEL, URI_TEMPLATE_LABEL, METHOD_COLUMN_LABEL, STATE_COLUMN_LABEL, ROUTE_COLUMN_LABEL));
                System.out.println(String.format(headerFormat, "---", "---------", "------------", "------", "-----", "--------"));
            } else {
                System.out.println(String.format(headerFormat, BASE_PATH_LABEL, URI_TEMPLATE_LABEL, METHOD_COLUMN_LABEL, STATE_COLUMN_LABEL, ROUTE_COLUMN_LABEL));
                System.out.println(String.format(headerFormat, "---------", "------------", "------", "-----", "--------"));
            }
            for (Map<String, String> row : services) {
                ...
                if (verbose) {
                    System.out.println(String.format(rowFormat, uri, basePath, uriTemplate, method, state, route));
                } else {
                    System.out.println(String.format(rowFormat, basePath, uriTemplate, method, state, route));
                }
            }
        }
        return null;
