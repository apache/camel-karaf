...
@Service
public class EndpointList extends CamelController implements Action {
    ...
    @Option(name = "--verbose", aliases = "-v", description = "Verbose output which does not limit the length of the uri shown, or to explain all options", required = false, multiValued = false, valueToShowInHelp = "false")
    boolean verbose;

    @Override
    public Object execute() throws Exception {
        final List<Map<String, String>> camelContextInfos = getCamelContexts(name);
        ...
        for (Map<String, String> camelContextInfo : camelContextInfos) {
            ...
            final List<Map<String, String>> endpoints = getEndpoints(camelContextName);
        ...
        for (Map.Entry<String, List<Map<String, String>>> stringListEntry : contextsToEndpoints.entrySet()) {
            ...
            System.out.println(String.format(headerFormat, CONTEXT_NAME_COLUMN_LABEL, URI_COLUMN_LABEL, STATUS_COLUMN_LABEL));
            ...
            for (Map<String, String> row : endpoints) {
                ...
                System.out.println(String.format(rowFormat, camelContextName, uri, state));
            }
        }
        return null;
    }
