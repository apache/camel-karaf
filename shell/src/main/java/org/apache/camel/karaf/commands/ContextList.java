...
@Service
public class ContextList extends CamelController implements Action {
    ...
    public Object execute() throws Exception {
        final List<Map<String, String>> camelContexts = this.getCamelContexts();
        ...
        if (camelContexts.size() > 0) {
            System.out.println(String.format(headerFormat, CONTEXT_COLUMN_LABEL, STATUS_COLUMN_LABEL, TOTAL_COLUMN_LABEL, FAILED_COLUMN_LABEL, INFLIGHT_COLUMN_LABEL, UPTIME_COLUMN_LABEL));
            System.out.println(String.format(headerFormat, "-------", "------", "-------", "--------", "----------", "------"));
            for (Map<String, String> row : camelContexts) {
                System.out.println(String.format(rowFormat, row.get("name"), row.get("state"), row.get("exchangesTotal"),
