...
import org.apache.camel.karaf.core.OsgiEventAdminNotifier;
...
public class CamelContextFactoryBean extends AbstractCamelContextFactoryBean<BlueprintCamelContext> {
    ...
    public BlueprintCamelContext getContext(boolean create) {
        if (context == null && create) {
            ...
            if (!isImplicitId()) {
                context.getCamelContextExtension().setName(getId());
            }
        }
        return context;
    }
    ...
    protected void findRouteBuildersByPackageScan(String[] packages, PackageScanFilter filter, List<RoutesBuilder> builders) throws Exception {
        ...
        getContext().getCamelContextExtension().getContextPlugin(PackageScanClassResolver.class).addFilter(filter);
        ...
        PackageScanRouteBuilderFinder finder = new PackageScanRouteBuilderFinder(getContext(), packages, classLoader,
                getContext().getCamelContextExtension().getContextPlugin(PackageScanClassResolver.class));
        finder.appendBuilders(builders);

        // and remove the filter
        getContext().getCamelContextExtension().getContextPlugin(PackageScanClassResolver.class).removeFilter(filter);
    }
